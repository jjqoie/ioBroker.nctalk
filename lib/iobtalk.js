// This module connects the iobroker side with the nextcloud talk side
// It is organized in iobroker custom groups which are mapped to the nextcloud chat instances identified by a token
// It is also responsible for creating the states for the groups
// It is also responsible for handling the events from the nextcloud side and sending the events to the iobroker side

class iobTalk {
    constructor(adapter, name, tokenlist) {
        this.adapter = adapter;
        name = (name || "").replace(adapter.FORBIDDEN_CHARS, "_");
        this.name = name;
    ...
    CreateStates() {
        ...
        this.adapter.setObjectNotExists(this.name + ".ShareFile", {
            type: "folder",
            common: {
    ...
    MessageReceived(msg) {
        if (msg[0].actorId.toLowerCase() == this.adapter.Talk.GetOwnActorIdLowerCase()) {
            this.adapter.log.debug("OWN MESSAGEEVENT " + msg[0].message);
        }
        else {
            this.adapter.log.debug("MESSAGEEVENT " + msg[0].message);
    ...
    RegisterEvents() {
        this.tokenlist.forEach(token => {
            this.adapter.log.debug(this.name, "Message_" + token);
        ...
        switch (statename) {
            ...
            case "LastReceivedMessage":
                if (state.ack == true) {
                    ...
                    this.adapter.log.debug("LastReceivedMessage was ack");
                }
                break;
            case "NextcloudPath":
                if (state.ack == false) {
            ...
            case "URL":
                if (state.ack == false) {
                    ...
                    this.adapter.log.debug(typeof (state.val));
                    this.adapter.log.debug(state.val);
